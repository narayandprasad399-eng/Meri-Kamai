    ...
    >
      {copied ? (
        <>
          ...
          <span className="text-[#00ff88] text-xs font-bold">Copied!</span>
        </>
      ) : (
        <>
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: `radial-gradient(circle at 80% 50%, ${meta.accent}10 0%, transparent 60%)`,
          opacity: hovered ? 1 : 0,
        }}
      />
      {app.is_featured && (
      ...
      <div className="relative z-10 p-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-xs font-black ${meta.accentText}`}>
              {app.earn_min && app.earn_max ? `₹${app.earn_min}–₹${app.earn_max}` : app.earn_min ? `₹${app.earn_min}` : 'Varies'}
            </span>
            {app.earn_note && <span className="text-[9px] text-gray-600">({app.earn_note})</span>}
          </div>
        </div>
        <a
        ...
        >
          {app.cta_text || 'Install'} <ArrowRight className="w-3.5 h-3.5" />
        </a>
      </div>
      {app.extra_info && (
    ...
    <div className="rounded-2xl border border-gray-800/60 overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${meta.badgeStyle}`}>
...
function CourseBanner({ referCode, checkoutUrl, subdomain }) {
  const [pulse, setPulse] = useState(false);

  // BUG FIXED HERE: Removed the extra semicolon that crashed your app!
  useEffect(() => {
    const t = setInterval(() => {
      setPulse(p => !p);
    }, 3000);
  ...
  // 🚀 FIX: DYNAMIC DEMO LINK LOGIC
  const mainDomain = window.location.origin.includes(`${subdomain}.`)
    ? window.location.origin.replace(`${subdomain}.`, '')
    : window.location.origin;

  return (
    <div
      className="relative rounded-3xl overflow-hidden border-2 border-blue-500/40"
      style={{ background: 'linear-gradient(135deg, #060d1f 0%, #0a0d1a 50%, #060d1f 100%)' }}
    >
      <div
        className="absolute top-0 right-0 w-48 h-48 rounded-full blur-[80px] transition-opacity duration-[3000ms]"
        style={{ background: '#3b82f620', opacity: pulse ? 0.8 : 0.3 }}
      />
      <div
        className="absolute bottom-0 left-0 w-32 h-32 rounded-full blur-[60px] transition-opacity duration-[3000ms]"
      ...
      <div className="relative z-10 p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {['🎬 Video Modules', '📱 Mobile+Desktop', '♾️ Lifetime Access', '📝 Daily Practice'].map((f, i) => (
            <span key={i} className="text-[10px] text-gray-300 bg-white/5 border border-white/10 px-2.5 py-1 rounded-full">
              {f}
            </span>
          ))}
        </div>
        ...
        >
          <Gift className="w-4 h-4" /> Buy Now @ ₹499 — Code Applied! <ArrowRight className="w-4 h-4" />
        </a>

        {/* Dynamic Link jo user ko proper main domain ke /course par bhejega */}
        <a
          href={`${mainDomain}/course`}
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#060809] gap-4">
      <div className="relative">
        <div className="w-16 h-16 rounded-2xl bg-[#00ff88] flex items-center justify-center font-black text-[#0a0a0a] text-2xl shadow-[0_0_40px_rgba(0,255,136,0.4)]">
          ₹
        </div>
  ...
  return (
    <div className="bg-[#0d1410] border border-[#00ff88]/12 rounded-2xl p-4">
      <div className="flex gap-1 mb-2">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className="w-3 h-3 fill-[#00ff88] text-[#00ff88]" />
        ))}
      </div>
          ...
          {TESTIMONIALS.map((_, i) => (
            <button
              ...
              className={`h-1 rounded-full transition-all duration-300 ${
                i === active ? 'w-5 bg-[#00ff88]' : 'w-1.5 bg-gray-700'
              }`}
            />
          ))}
        </div>
...
export default function UserSite({ customSubdomain }) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // 🚀 FIX: FETCH ALL MASTER APPS + USER APPS FOR FALLBACK LOGIC
        ...
        (masterApps || []).forEach(app => {
          const userConfig = (userApps || []).find(ua => ua.app_id === app.id);
          
          // Agar user ne app ko hide (remove) kar diya hai, toh usko site par mat dikhao
          if (userConfig && userConfig.is_active === false) return;

          // Agar user ne apna link daala hai toh wo use karo, warna Admin ka default link use karo
          const finalUrl = (userConfig && userConfig.is_active && userConfig.referral_url) 
            ? userConfig.referral_url 
            : (app.admin_link || 'https://merikamai.in');

          const cat = app.category;
          if (!grouped[cat]) grouped[cat] = [];
          grouped[cat].push({ ...app, referral_url: finalUrl });
        });
  ...
  if (!shopOwner) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#060809] text-white gap-4 px-4 text-center">
        ...
        <a href="https://merikamai.in" className="text-[#00ff88] font-bold hover:underline flex items-center gap-1">
  ...
  return (
    <div className="min-h-screen bg-[#060809] text-[#e5e5e5] overflow-x-hidden pb-16" style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
      
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] h-[300px] rounded-full blur-[120px]" style={{ background: 'rgba(0,255,136,0.04)' }} />
        <div className="absolute top-[40%] right-0 w-[300px] h-[300px] rounded-full blur-[100px]" style={{ background: 'rgba(59,130,246,0.04)' }} />
      </div>
      
      <div className="relative z-10 pt-8 pb-6 px-4 text-center border-b border-white/5" style={{ background: 'linear-gradient(to bottom, #0a0f0b, #060809)' }}>
        <div className="relative inline-block mb-4">
          <div className="w-20 h-20 mx-auto rounded-3xl flex items-center justify-center font-black text-[#0a0a0a] text-3xl shadow-[0_0_40px_rgba(0,255,136,0.35)]" style={{ background: 'linear-gradient(135deg, #00ff88, #00cc66)' }}>
        ...
        <h1 className="text-2xl sm:text-3xl font-black text-white mb-1"><span className="text-[#00ff88]">{ownerFirst}'s</span> Deals & Offers</h1>
        <p className="text-gray-500 text-sm mb-4 max-w-xs mx-auto">Exclusive earning apps, cashback deals aur English Course — sab ek jagah!</p>
        ...
        {socialLinks && (
          <div className="flex justify-center gap-2.5">
            {socialLinks.youtube_link && <a href={socialLinks.youtube_link} target="_blank" rel="noreferrer" className="w-9 h-9 bg-[#0d1410] border border-gray-800 rounded-xl flex items-center justify-center text-gray-500 hover:text-red-500 hover:border-red-500/30 transition-all"><FaYoutube className="w-4 h-4" /></a>}
            {socialLinks.instagram_link && <a href={socialLinks.instagram_link} target="_blank" rel="noreferrer" className="w-9 h-9 bg-[#0d1410] border border-gray-800 rounded-xl flex items-center justify-center text-gray-500 hover:text-pink-500 hover:border-pink-500/30 transition-all"><FaInstagram className="w-4 h-4" /></a>}
            {socialLinks.telegram_link && <a href={socialLinks.telegram_link} target="_blank" rel="noreferrer" className="w-9 h-9 bg-[#0d1410] border border-gray-800 rounded-xl flex items-center justify-center text-gray-500 hover:text-blue-400 hover:border-blue-500/30 transition-all"><FaTelegramPlane className="w-4 h-4" /></a>}
            {socialLinks.whatsapp_link && <a href={socialLinks.whatsapp_link} target="_blank" rel="noreferrer" className="w-9 h-9 bg-[#0d1410] border border-gray-800 rounded-xl flex items-center justify-center text-gray-500 hover:text-green-400 hover:border-green-500/30 transition-all"><FaWhatsapp className="w-4 h-4" /></a>}
            <button onClick={handleShare} className="w-9 h-9 bg-[#0d1410] border border-gray-800 rounded-xl flex items-center justify-center text-gray-500 hover:text-[#00ff88] hover:border-[#00ff88]/30 transition-all">
      ...
      <div className="relative z-10 max-w-xl mx-auto px-4 mt-6 space-y-5">
        <div>
          ...
          <CourseBanner referCode={shopOwner.refer_code} checkoutUrl={checkoutUrl} subdomain={subdomain} />
        </div>
        ...
        {hasAnySections ? (
          <div>
            ...
            <div className="space-y-3">
              {categoryOrder.map((cat, i) => appsByCategory[cat]?.length > 0 ? <AppSection key={cat} categoryKey={cat} apps={appsByCategory[cat]} defaultOpen={i === 0} /> : null)}
            </div>
          </div>
        ) : (
          <div className="text-center py-8 border border-dashed border-gray-800 rounded-2xl">
            <p className="text-3xl mb-2">🛍️</p>
            <p className="text-gray-500 text-sm">No apps available right now.</p>
          </div>
        )}
        ...
        <div className="bg-[#0d1410] border border-[#00ff88]/10 rounded-2xl p-4 flex gap-3">
          <div className="w-10 h-10 rounded-xl shrink-0 flex items-center justify-center font-black text-black text-sm" style={{ background: 'linear-gradient(135deg, #00ff88, #00cc66)' }}>
            {ownerFirst.charAt(0).toUpperCase()}
          </div>
          <div>
            <p className="text-white font-bold text-sm mb-1">{ownerFirst} recommends:</p>
            <p className="text-gray-400 text-xs leading-relaxed">
              "Maine yeh sab apps khud try kiye hain — genuine earning hoti hai. Course ke liye code <span className="font-black text-[#00ff88]">{shopOwner.refer_code}</span> use karo, ₹500 off milega! Koi sawaal ho toh message karo."
            </p>
          </div>
        </div>
        ...
        <div className="text-center pt-2 pb-2">
          ...
          <a href="https://merikamai.in" className="inline-flex items-center gap-1.5 group">
            <div className="w-5 h-5 rounded bg-[#00ff88] flex items-center justify-center font-black text-black text-[10px]">₹</div>
            <span className="font-black text-sm text-gray-600 group-hover:text-[#00ff88] transition-colors">Meri<span className="text-[#00ff88]">Kamai</span></span>
          </a>
        </div>
      </div>
    </div>
